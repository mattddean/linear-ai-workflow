...
Junior is the default reference for repository tooling, package scripts, workspace configuration, CI, and test setup as well as Effect architecture. Prefer changes that bring the repositories into alignment. When a copied Junior check fails because its supporting script, configuration, or dependency is missing, bring over the applicable setup instead of removing or bypassing the check. Keep equivalent workflow jobs, shared setup actions, and commands consistent. Limit differences to demonstrated project requirements or installed API differences, and explain those differences in the completion report.
...
- Compose shared services in `src/runtime/layers/root.ts`. Follow the [Josiah root runtime](../../josiah/tests/src/lib/runtime/layers/root.ts) pattern: named base/service layers and exported `RootLayer`. Construct the single module-level `rootRuntime` in `src/runtime/root.ts` and dispose it at the process boundary. Mirror this separation in `src/test/runtime/layers/root.ts` and `src/test/runtime/root.ts`; the test preload owns test runtime initialization and disposal. Domain services retain Effect requirements and never invoke the runtime themselves.
