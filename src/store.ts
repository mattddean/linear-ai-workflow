...
>()('linear-ai-workflow/store/Store') {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const get = Effect.fn('Store.get')(function* (id: RunId) {
        ...
        return yield* Schema.decodeUnknownEffect(RunData)(row.data)
      ...
      return Store.of({
        ...
        list: db
          ...
          .pipe(
            Effect.flatMap((rows) => Schema.decodeUnknownEffect(Schema.Array(RunData))(rows.map((row) => row.data))),
            Effect.mapError(storageError),
          ),
        journal: Effect.fn('Store.journal')(function* (run) {
          ...
          return rows[0] ? yield* Schema.decodeUnknownEffect(JournalData)(rows[0].data) : null
        }, Effect.mapError(storageError)),
