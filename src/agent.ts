...
>()('linear-ai-workflow/agent/Agent') {
      ...
      return Agent.of({
        execute: Effect.fn('Agent.execute')(function* (assignment) {
          ...
          return yield* Effect.gen(function* () {
            ...
            const resultSchema = JsonSchema.toDocumentDraft07(
              Schema.toJsonSchemaDocument(Result, { onExcessProperty: 'error' }),
            )
            yield* io(() =>
              writeFile(schemaPath, JSON.stringify({ ...resultSchema.schema, definitions: resultSchema.definitions })),
            )
            const wheyCommand = [
              process.execPath,
            ...
            const result = yield* Schema.decodeUnknownEffect(Schema.fromJsonString(Result))(raw).pipe(
