...
export const GraphQLClientLive = (
  endpoint: string,
    ...
    Effect.gen(function* () {
      ...
      const execute = Effect.fn('GraphQLClient.execute')(function* <A, I>(input: GraphQLRequest<A, I>) {
        ...
        const response = yield* client.execute(request).pipe(
          ...
          Effect.timeoutFail({
            duration: '30 seconds',
            onTimeout: () => error('transport', 'GraphQL request timed out; reconcile before retrying'),
          }),
        )
        ...
        const text = yield* response.text.pipe(
          ...
          Effect.timeoutFail({
            duration: '30 seconds',
            onTimeout: () => error('transport', 'GraphQL response body timed out'),
          }),
        )
        ...
        return yield* Schema.decodeUnknown(input.schema)(envelope.data).pipe(
          Effect.mapError(() => error('linear', 'Invalid GraphQL operation data; reconcile mutations before retrying')),
        )
      })
